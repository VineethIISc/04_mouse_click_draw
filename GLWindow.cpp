...
GLWindow::GLWindow():screenWidth(0), screenHeight(0)
{
	...
	//InteractiveTriangle
	InteractiveTriangle = new InteractiveDraw;
}
...
void GLWindow::sendDataToGPU()
{
	...
	//assign memory for incoming data
	InteractiveTriangle->allocateMemory();	
}
...
void GLWindow::drawObjects()
{
	...
	//render interactive triangle
	InteractiveTriangle->render();
}
...
void GLWindow::mousePressEvent(QMouseEvent* buttonPress)
{
	...
	if (buttonPress->button() == Qt::LeftButton)
	{
		...
		screenToClip(mPos);
		InteractiveTriangle->setVertexPosition(mPos);
	}
